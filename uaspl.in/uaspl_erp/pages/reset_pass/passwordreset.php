...
if(isset($_REQUEST['pwdrst']))
{
  ...
  if($pwd == $cpwd)
  {
    $reset_pwd = mysqli_query($connection,"update admin set password='$pwd' where email='$email'");
    if($reset_pwd>0)
    {
 ...
 if($_GET['secret'])
  ...
  $email = base64_decode($_GET['secret']);
   $check_details = mysqli_query($connection,"select email from admin where email='$email'");
